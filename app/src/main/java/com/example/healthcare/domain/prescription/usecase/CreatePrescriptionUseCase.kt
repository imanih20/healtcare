...
class CreatePrescriptionUseCase(private val service: PrescriptionService) {
    suspend operator fun invoke(
        prescriptionList: List<PrescriptionRequest>
    ) : Flow<BaseResult<List<Prescription>,WrappedListResponse<PrescriptionResponse>>> {
        val response = service.createPrescription(prescriptionList)
